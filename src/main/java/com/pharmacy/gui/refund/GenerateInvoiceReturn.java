...
public class GenerateInvoiceReturn extends JDialog {
	...
	public void setInitData(InvoiceReturn invoiceReturn, List<RefundReqDTO> listChosen) {
		lblValueMaHD.setText(invoiceReturn.getMaHDTra());
		lblValueNgayLap.setText(FormatUtil.formatDate(invoiceReturn.getNgayLap()));
		lblValueNhanVien.setText(invoiceReturn.getEmployee().getDisplayName());
		lblMoney.setText(FormatUtil.formatVND(invoiceReturn.getTienHoan()));

		String text = "<html><p style='text-align:right;'>"
				+ VietnameseUtil.convertNumberToTextVND((long) invoiceReturn.getTienHoan()) + "</p></html>";

		lblDesc.setText(text);

		String sdt = invoiceReturn.getCustomer().getSoDienThoai();
		lblValueSDT.setText(sdt.substring(0, 4) + "***" + sdt.substring(7));
		lblValueTenKH.setText(invoiceReturn.getCustomer().getTenKhachHang());

		showChiTietHoaDon(listChosen);

		try {
			BufferedImage img = QRGenerator.generateQRCodeImage(invoiceReturn.getMaHDTra(), 120, 120);
	...
	private void showChiTietHoaDon(List<RefundReqDTO> list) {
	...
	private JPanel initProdDetail(RefundReqDTO refund) {
		JPanel pnl = new JPanel();
		pnl.setLayout(new MigLayout("wrap 5, fillx", "[38%][fill][fill][fill][fill]", "[]"));

		JLabel lblTitle1 = new JLabel(refund.getTenSP());
		JLabel lblTitle2 = new JLabel(refund.getDonViTinh());
		JLabel lblTitle3 = new JLabel(String.valueOf(refund.getSoLuongTra()));
		JLabel lblTitle4 = new JLabel(FormatUtil.formatVND(refund.getDonGia()));
		JLabel lblTitle5 = new JLabel(FormatUtil.formatVND(refund.getThanhTien()));
