package com.pharmacy.gui.refund;
...
import com.pharmacy.utils.CustomQuantitySpinner;
import com.pharmacy.utils.FormatUtil;

public class ReturnProductGUI extends JPanel {

	private final RefundBUS refundBUS;

	private DefaultTableModel modelSanPham;

	private DefaultTableModel modelHoaDon;

	private List<RefundReqDTO> listChosen = new ArrayList<>();

	private List<InvoiceDetail> listAll = new ArrayList<>();
	...
	private void initEvent() {
		txtSearch.addActionListener(e -> {
			...
			try {
				invoice = refundBUS.getInvoiceById(qrCode);
				loadData();
				listAll = refundBUS.getAllInvoiceDetailByQrCode(qrCode);
				viewTableHoaDon();
			...
			if (e.getType() == javax.swing.event.TableModelEvent.UPDATE && e.getColumn() == 3) {
				...
				SwingUtilities.invokeLater(() -> {
					try {
						...
						double donGia = listAll.get(row).getDonGia();
	...
	private void createInvoiceReturn() {
		...
		invoiceReturn.setCustomer(invoice.getCustomer());
		invoiceReturn.setEmployee(Auth.getCurrentUser());
		invoiceReturn.setNgayLap(LocalDateTime.now());
		invoiceReturn.setTienHoan(total);
		invoiceReturn.setLyDo((String) comboTinhTrang.getSelectedItem() + ": " + txtLyDo.getText());

		try {
			refundBUS.processInvoiceReturn(invoiceReturn, listChosen);
		...
		if (ok == JOptionPane.YES_OPTION) {
			...
			List<RefundReqDTO> res = new ArrayList<>(
					listChosen.stream().collect(Collectors.toMap(RefundReqDTO::getMaSP, p -> p, (p1, p2) -> {
						p1.setSoLuongTra(p1.getSoLuongTra() + p2.getSoLuongTra());
						p1.setThanhTien();

						return p1;
					})).values());

			generateInvoice.setInitData(invoiceReturn, res);
	...
	private void addInvDetailForRefund() {
		...
		for (int i = 0; i < modelHoaDon.getRowCount(); i++) {
			...
			if (checked) {
				InvoiceDetail originalDetail = listAll.get(i);
				...
				double thanhTienHoanTra = soLuongHoanTra * originalDetail.getDonGia();

				listChosen.add(new RefundReqDTO(originalDetail.getProduct().getMaSanPham(),
						originalDetail.getShipment().getSoLo(), soLuongHoanTra, originalDetail.getDonGia(),
						thanhTienHoanTra, originalDetail.getProduct().getDonViTinh(),
						originalDetail.getProduct().getTenSanPham()));

				modelSanPham.addRow(new Object[] { index++, originalDetail.getProduct().getTenSanPham(),
						originalDetail.getShipment().getSoLo(), soLuongHoanTra });
			}
		}
	...
	private void calculatorThanhTien() {
		total = listChosen.stream().mapToDouble(RefundReqDTO::getThanhTien).sum();
	...
	private void loadData() {
		txtMaNV.setText(Auth.getCurrentUser().getMaNhanVien());
		txtTenNV.setText(Auth.getCurrentUser().getTenNhanVien());
		txtMaHD.setText(invoice.getMaHoaDon());
		txtNgayMua.setText(FormatUtil.formatDate(invoice.getNgayLap()));
		txtSdt.setText(invoice.getCustomer().getSoDienThoai());
		txtTenKH.setText(invoice.getCustomer().getTenKhachHang());
	...
	private void viewTableHoaDon() {
		...
		for (InvoiceDetail invoice : listAll)
			modelHoaDon.addRow(invoice.getObjects());
	}
...
class QuantityCellEditor extends AbstractCellEditor implements TableCellEditor {
	...
	private java.util.List<InvoiceDetail> listAll;

	public QuantityCellEditor(java.util.List<InvoiceDetail> listAll) {
	...
	public void setListInvoice(java.util.List<InvoiceDetail> listAll) {
	...
	public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
		...
		if (listAll != null && row < listAll.size()) {
			InvoiceDetail currentDetail = listAll.get(row);

			int maxQty = currentDetail.getSoLuong();
