...
public class AddProdDialog extends javax.swing.JDialog {
	...
	private static final Logger log = LoggerFactory.getLogger(AddProdDialog.class);
	...
	public AddProdDialog(java.awt.Frame parent, boolean modal, ProductGUI productGUI, ProductBUS productBUS) {
		super(parent, modal);
	...
	// xem docs bên class EditProdDialog
	...
	private void addNewProd() {
		try {
		    ...
		    prod.setQuyCachDongGoi(quyCach);
		    ...
		    prod.setChongChiDinh(chongChiDinh);
		    ...
		    prod.setLieuDung(lieuDung);

		    prod.setLoaiSanPham(new ProductType(cmbLoai.getSelectedIndex() + 1));
		    prod.setMoTa(txaMoTa.getText());

			// check barcode have exists
			if (productBUS.checkExistsBarcode(barcode)) {
				JOptionPane.showMessageDialog(this, "Barcode này đã tồn tại!", "Error", JOptionPane.ERROR_MESSAGE);
				txtBarcode.selectAll();
				txtBarcode.requestFocus();
				return;
			}

			// check SDK have exists
			if (productBUS.checkExistsSoDangKy(soDangKy)) {
			...
			prod.setAvatarUrl(String.format("%s.%s", barcode, SupportImageUtil.getFileExtension(tempAvatarFileName)));

			log.info("[GUI] Product information before adding: {}", prod.toStringTmp());

			// save product to db
			productBUS.addProd(prod);
			...
			productGUI.showLastPage();

			log.info("[GUI] Product added successfully with id: {}.", prod.getMaSanPham());
	...
	private void initComponents() {
		...
		lblAvatar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/prod/default.png"))); // NOI18N
