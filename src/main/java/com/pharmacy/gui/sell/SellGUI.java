...
import com.pharmacy.bus.SellBUS;
...
import com.pharmacy.utils.SuggestMoney;
...
public class SellGUI extends javax.swing.JPanel {
	...
	private List<ProductBatchesReqDTO> list;

	private final SellBUS sellBUS;

	private final CustomerBUS customerBUS;

	private final VoucherBUS voucherBUS;
	...
	public SellGUI() {
		...
		this.voucherBUS = new VoucherBUS();
	...
	private void setTongTienForUI() {
		...
		tongTienHang = sellBUS.calculateTongTienHang(list);

		loadVoucher();

		soTienCanThanhToan = sellBUS.calculateTongCanThanhToan(tongTienHang, soTienDuocGiam);
	...
	private void initEvent() {
		...
		txtSearch.addActionListener(e -> {
			...
			try {
				ProductBatchesReqDTO productBatchesReqDTO = sellBUS.getBatchesByBarcode(barcode);

				Optional<ProductBatchesReqDTO> existedItem = list.stream()
						.filter(item -> item.getMaSanPham() == productBatchesReqDTO.getMaSanPham()).findFirst();

				if (existedItem.isPresent()) {
					ProductBatchesReqDTO existing = existedItem.get();
					sellBUS.processAddToCart(existing, existing.getSoLuongBan() + 1);

				} else {
					sellBUS.processAddToCart(productBatchesReqDTO, 1);
					list.add(productBatchesReqDTO);
				}
	...
	private void eventThanhToan() {
		...
		if (!sdt.isEmpty() || !tenKH.isEmpty()) { // nếu có nhập số điện thoại hoặc tên thì phải nhập đủ hết
			...
			customer = new Customer(tenKH, sdt);
			customer.setTangDiemTichLuy(tongTienHang);
		...
		if (cmbVoucher.getItemCount() > 0)
			voucher = (Voucher) cmbVoucher.getSelectedItem();
		...
		try {
			...
			invoice.setEmployee(Auth.getCurrentUser());
			invoice.setVoucher(voucher);
			invoice.setGhiChu(txaGhiChu.getText());
			invoice.setNgayLap(LocalDateTime.now());
			invoice.setTongTienHang(tongTienHang);
			invoice.setTongTienCanThanhToan(soTienCanThanhToan);

			sellBUS.processPayment(invoice, list);
			...
			if (ok == JOptionPane.YES_OPTION) {
				...
				generateInvoice.setInitData(invoice, list);
	...
	private void showProdChosen() {
		...
		for (ProductBatchesReqDTO productBatchesReqDTO : list) {
			pnlItemChosen.add(initCardItemChosen(productBatchesReqDTO, count++));
		}
	...
	private void handleTienThua() {
		...
		txtTienThua.setText(FormatUtil.formatVND(sellBUS.calculateTienThua(s, soTienCanThanhToan)) + " VND");
	}

	private void loadSoTienDuocGiam() {
		Voucher voucher = (Voucher) cmbVoucher.getSelectedItem();
		...
		soTienDuocGiam = voucher.getTongTienDuocGiam();
	...
	public void handleSearchCustomer() {
		...
		customer = customerBUS.searchCustomerByPhone(phone);
		...
		txtHotenKH.setText(customer.getFullName());
	...
	private void loadVoucher() {
		...
		List<Voucher> listVoucher = voucherBUS.getAllVoucherByConditionCus(customer, tongTienHang);
	...
	private JPanel initCardItemChosen(ProductBatchesReqDTO prod, int index) {
		JPanel pnl = new JPanel();
		pnl.setLayout(new MigLayout("wrap, fill", "[grow 0][grow 0][fill]", "[][]"));

		JLabel lblIcon = new JLabel(HelperImageIcon.scaleIcon(prod.getAvatarUrl(), WIDTH_IMAGE, HEIGHT_IMAGE));

		JLabel lblName = new JLabel(String.format("%d. %s", index, prod.getTenSanPham()));
		...
		JSpinner spinner = new JSpinner(new SpinnerNumberModel(prod.getSoLuongBan(), 1, 1000, 1));

		JLabel lblUnit = new JLabel(prod.getDonViTinh());

		JLabel lblGiaBan = new JLabel(FormatUtil.formatVND(prod.getGiaBanThucTe()));

		JLabel lblPrice = new JLabel(FormatUtil.formatVND(prod.getThanhTien()) + " đ");
		...
		btnDel.addActionListener(e -> removeProdItemChosen(prod, pnl));

		btnSoLo.addActionListener(e -> viewPhanBoLo(prod));

		spinner.addChangeListener(e -> handleForSpinner(spinner, prod, lblGiaBan, lblPrice));

		return pnl;
	}

	private void viewPhanBoLo(ProductBatchesReqDTO prod) {
		ViewPhanBoLo view = new ViewPhanBoLo(null, false, prod);
		view.setLocationRelativeTo(null);
		view.setVisible(true);
	}

	private void handleForSpinner(JSpinner spinner, ProductBatchesReqDTO prod, JLabel lblGiaBan, JLabel lblThanhTien) {
		int newValue = (int) spinner.getValue();

		try {
			sellBUS.processAddToCart(prod, newValue);
		} catch (IllegalArgumentException e) {
			spinner.setValue(prod.getSoLuongBan());
			JOptionPane.showMessageDialog(this, e.getMessage());
			return;
		}

		lblGiaBan.setText(FormatUtil.formatVND(prod.getGiaBanThucTe()));
		lblThanhTien.setText(FormatUtil.formatVND(prod.getThanhTien()) + " đ");

		setTongTienForUI();
	}

	private void removeProdItemChosen(ProductBatchesReqDTO prod, JPanel pnlToRemove) {
		list.remove(prod);
	...
	private javax.swing.JComboBox<Voucher> cmbVoucher;
