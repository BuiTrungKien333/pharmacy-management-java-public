...
import com.pharmacy.utils.QRGenerator;
...
public class GenerateInvoice extends JDialog {
	...
	public void setInitData(Invoice invoice, List<ProductBatchesReqDTO> list) {
		lblValueMaHD.setText(invoice.getMaHoaDon());
		lblValueNgayLap.setText(FormatUtil.formatDate(invoice.getNgayLap()));
		lblValueNhanVien.setText(invoice.getEmployee().getDisplayName());
		lblMoney.setText(FormatUtil.formatVND(invoice.getTongTienHang()));
		lblMoneyChietKhau.setText(FormatUtil.formatVND(invoice.getTongTienHang() - invoice.getTongTienCanThanhToan()));
		lblMoneyThanhToan.setText(FormatUtil.formatVND(invoice.getTongTienCanThanhToan()));

		String text = "<html><p style='text-align:right;'>"
		        + VietnameseUtil.convertNumberToTextVND((long) invoice.getTongTienCanThanhToan())
		        + "</p></html>";
		...
		if (invoice.getCustomer() == null) {
			...
			String sdt = invoice.getCustomer().getSoDienThoai();
			lblValueSDT.setText(sdt.substring(0, 4) + "***" + sdt.substring(7));
			lblValueTenKH.setText(invoice.getCustomer().getTenKhachHang());
		}

		showChiTietHoaDon(list);

		try {
			BufferedImage img = QRGenerator.generateQRCodeImage(invoice.getMaHoaDon(), 120, 120);
	...
	private void showChiTietHoaDon(List<ProductBatchesReqDTO> list) {
	...
	private JPanel initProdDetail(ProductBatchesReqDTO prod) {
		JPanel pnl = new JPanel();
		pnl.setLayout(new MigLayout("wrap 5, fillx", "[fill][fill][fill][fill][fill]", "[]"));

		JLabel lblTitle1 = new JLabel(prod.getTenSanPham());
		JLabel lblTitle2 = new JLabel(prod.getDonViTinh());
		JLabel lblTitle3 = new JLabel(String.valueOf(prod.getSoLuongBan()));
		JLabel lblTitle4 = new JLabel(FormatUtil.formatVND(prod.getGiaBanThucTe()));
		JLabel lblTitle5 = new JLabel(FormatUtil.formatVND(prod.getThanhTien()));
