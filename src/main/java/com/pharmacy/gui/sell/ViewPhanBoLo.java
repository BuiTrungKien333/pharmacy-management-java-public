...
public class ViewPhanBoLo extends javax.swing.JDialog {
	
	public ViewPhanBoLo(java.awt.Frame parent, boolean modal, ProductBatchesReqDTO prod) {
		super(parent, modal);
		initComponents();
	...
	public void initData(ProductBatchesReqDTO prod) {
		lblValueTenSP.setText(prod.getTenSanPham());
		lblValueSLCon.setText(prod.getTongSoLuong() + " " + prod.getDonViTinh());
		lblValueSLBan.setText(prod.getSoLuongBan() + " " + prod.getDonViTinh());
		lblValueGiaBan.setText(FormatUtil.formatVND(prod.getGiaBanThucTe()) + " đ");
		lblValueThanhTien.setText(FormatUtil.formatVND(prod.getThanhTien()) + " đ");
		
		viewTable(prod.getChiTietGiaoDich());
	}

	private void viewTable(List<PhanBoLoDTO> list) {
		model = (DefaultTableModel) tbl.getModel();
		model.setRowCount(0);
		
		for(PhanBoLoDTO pbl : list) {
			model.addRow(pbl.getObjects());
		}
	}
