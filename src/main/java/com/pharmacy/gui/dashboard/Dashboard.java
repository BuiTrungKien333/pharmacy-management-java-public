package com.pharmacy.gui.dashboard;
...
import com.pharmacy.bus.DashboardBUS;
...
public class Dashboard extends JPanel {
	...
	private void initHeader() {
		...
		pnlHeader.add(createCardPanel("icon/svg/money-bag.svg", "dashboard.card.today_revenue",
				FormatUtil.formatVND(dashboardBUS.getCalculateDailyRevenue()), "dashboard.card.diff_yesterday"));

		pnlHeader.add(createCardPanel("icon/svg/order.svg", "dashboard.card.orders",
				String.valueOf(dashboardBUS.getCountDailyInvoices()), "dashboard.card.order_type"));

		pnlHeader.add(createCardPanel("icon/svg/chart.svg", "dashboard.card.estimate_profit",
				FormatUtil.formatVND(dashboardBUS.getCalculateDailyProfit()), "dashboard.card.estimate"));

		pnlHeader.add(createCardPanel("icon/svg/user-check.svg", "dashboard.card.new_customer",
				String.valueOf(dashboardBUS.getCountDailyNewCustomers()), "dashboard.card.member"));
	...
	private void createLineChart() {
		...
		Map<String, Double> data = dashboardBUS.getRevenueLast7Days();
...
//		data.forEach((x, y) -> dataset.addValue(y, seriesName, x));
	...
	private JPanel createBarChartPanel() {
		...
		Map<String, Integer> mpBan = dashboardBUS.getSalesInvoiceCountLast5Days();
		Map<String, Integer> mpTra = dashboardBUS.getReturnInvoiceCountLast5Days();
...
class CardItem {
	...
	CardItem(JLabel lblTitle, JLabel lblValue, JLabel lblSub, String keyTitle, String keySub) {
